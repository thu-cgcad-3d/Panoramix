...
struct PanoramixOptions {
    ...
    void serialize(Archiver & ar) {
        ar( useWallPrior, 
            usePrincipleDirectionPrior, 
            useGeometricContextPrior, useGTOcclusions, 
            looseLinesSecondTime, 
            looseSegsSecondTime, 
            restrictSegsSecondTime);
        ar( refresh_preparation, 
            refresh_mg_init, 
            refresh_line2leftRightSegs, 
            refresh_mg_oriented, 
            refresh_lsw, 
            refresh_mg_occdetected, 
...
struct PanoramixReport {
    ...
    void serialize(Archiver & ar) {
        ar( time_preparation,
            time_mg_init,
            time_line2leftRightSegs,
            time_mg_oriented,
            time_lsw,
            time_mg_occdetected,
            time_mg_reconstructed,
            time_solve_lp,
            succeeded);
    }
};
...
std::vector<Image3d> GetSurfaceNormalMapsOfPanoramix(
    ...
    if (!GetPanoramixResult(anno, options, mg, cg, dp)) {
        std::cout << "failed to load panoramix result, performing RunPanoramix ..." << std::endl;
...
template <class CameraT>
std::vector<Image3d> GetSurfaceDepthMapsOfPanoramix(
    const std::vector<CameraT> & testCams,
    const PILayoutAnnotation & anno,
    const PanoramixOptions & options,
    misc::Matlab & matlab) {
    ...
    ParallelRun(testCams.size(), std::thread::hardware_concurrency(), [&](int i) {
        ...
        auto & cam = testCams[i];
        map = SurfaceDepthMap(cam, dp, cg, mg, nullptr);
    });
