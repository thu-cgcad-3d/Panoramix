...
namespace third_party
{
  ...
  public:
    ...
    int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], CallbackFunctorT callback);
  ...
  protected:
    ...
    template <class CallbackFunctorT>
    bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, CallbackFunctorT callback);
  ...
  RTREE_TEMPLATE template <class CallbackFunctorT> 
  int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], CallbackFunctorT callback)
  {
#ifdef _DEBUG
      for (int index = 0; index < NUMDIMS; ++index)
      {
  ...
  RTREE_TEMPLATE template <class CallbackFunctorT>
      bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, CallbackFunctorT callback)
  {
      ASSERT(a_node);
      ASSERT(a_node->m_level >= 0);
      ASSERT(a_rect);
      ...
      if (a_node->IsInternalNode()) // This is an internal node in the tree
      {
          for (int index = 0; index < a_node->m_count; ++index)
          {
              if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
              {
                  if (!Search<CallbackFunctorT>(a_node->m_branch[index].m_child, a_rect, a_foundCount, callback))
                  {
                      return false; // Don't continue searching
                  }
              }
