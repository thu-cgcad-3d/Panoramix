...
namespace pano {
    namespace core {
        ...
        std::vector<int> ComputeSpatialRegionPropertiesTemplated(const Imagei & segmentedRegions, const CameraT & cam,
            ...
            if (ncontoursPtr){ ncontoursPtr->clear(); ncontoursPtr->reserve(regionNum); }
        ...
        View<PartialPanoramicCamera, Imageub> PerfectRegionMaskView(const std::vector<std::vector<Vec3>> & contours,
            ...
            PartialPanoramicCamera ppc(ppcSize, ppcSize, focal, Point3(0, 0, 0), ncenter, x);
