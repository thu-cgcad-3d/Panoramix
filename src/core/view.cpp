    ...
    namespace core {
        ...
        View<PanoramicCamera> CreatePanoramicView(const Image & panorama) {
            ...
            return View<PanoramicCamera>{
                panorama, 
                PanoramicCamera(panorama.cols / M_PI / 2.0, Point3(0, 0, 0), Vec3(1, 0, 0), Vec3(0, 0, 1))
            };
        }
        ...
        OrientationContext::OrientationContext(SizeI size){
            for (auto & op : probs){
                op = Imaged::zeros(size);
            }
        }

        std::pair<OrientationContext::OrientationImage, Imaged> OrientationContext::labelsAndConfidences() const {
            auto size = probs[0].size();
            OrientationContext::OrientationImage label(size);
            Imaged conf(size);
            for (int i = 0; i < size.width; i++){
                for (int j = 0; j < size.height; j++){                   
                    double values[OrientationNum];
                    for (int k = 0; k < probs.size(); k++){
                        double value = probs[k](j, i);
                        values[k] = value;
                    }
                    label(j, i) = (OrientationContext::Orientation)
                        (std::max_element(std::begin(values), std::end(values)) - std::begin(values));
                    double negEntropy = - EntropyOfContainer(values, 1.0);
                    conf(j, i) = negEntropy;
                }
            }
            return std::make_pair(label, conf);
        }


        namespace {

            inline void InstallGeometricContext(std::map<OrientationContext::Orientation, std::vector<Imaged>> & ops,
                const GeometricContextEstimator::Feature & gc,
                OrientationContext::Orientation to, std::initializer_list<GeometricContextLabel> froms){
                for (auto from : froms){
                    if (Contains(gc, from)){
                        ops[to].push_back(gc.at(from));
                    }
                }
            }
            ...
            inline void InstallGeometricContext(std::map<OrientationContext::Orientation, std::vector<Imaged>> & ops,
                const GeometricContextEstimator::Feature & gc){
                InstallGeometricContext(ops, gc, OrientationContext::Vertical, {
                    GeometricContextLabel::Front, GeometricContextLabel::Left, GeometricContextLabel::Right,
                    GeometricContextLabel::Vertical
                });

                InstallGeometricContext(ops, gc, OrientationContext::Downward, {
                    GeometricContextLabel::Ceiling
                });

                InstallGeometricContext(ops, gc, OrientationContext::Upward, {
                    GeometricContextLabel::Floor, GeometricContextLabel::Ground
                });

                InstallGeometricContext(ops, gc, OrientationContext::Void, {
                    GeometricContextLabel::Sky
                });

                InstallGeometricContext(ops, gc, OrientationContext::PlanarObject, {
                    GeometricContextLabel::Furniture
                });

                InstallGeometricContext(ops, gc, OrientationContext::NonPlanarObject, {
                    GeometricContextLabel::NotPlanar
                });
            }

            enum class MergePolicy { Max, Sum, MaxAndMean };
            inline OrientationContext MergeAndMakeOrientationContext(const SizeI & size, 
                std::map<OrientationContext::Orientation, std::vector<Imaged>> & ops,
                MergePolicy mp = MergePolicy::Sum){
                
                OrientationContext oc(size);

                for (auto & op : ops){
                    auto & dists = op.second;
                    Imaged & sum = oc.probs[op.first];
                    for (auto & dist : dists){
                        if (mp == MergePolicy::Max){
                            sum = cv::max(sum, dist);
                        }
                        else /*if (mp == MergePolicy::Sum)*/{
                            sum += dist;
                        }
                    }
                }
        ...
        namespace {
            ...
            inline OrientationContext::Orientation ToOrientation(GeometricContextLabel label){
                switch (label){
                case GeometricContextLabel::Ceiling: return OrientationContext::Downward;
                case GeometricContextLabel::Floor: return OrientationContext::Upward;
                case GeometricContextLabel::Front: 
                case GeometricContextLabel::Left:
                case GeometricContextLabel::Right:
                    return OrientationContext::Vertical;
                case GeometricContextLabel::Furniture: return OrientationContext::PlanarObject;
                case GeometricContextLabel::Ground: return OrientationContext::Upward;
                case GeometricContextLabel::Sky: return OrientationContext::Void;
                case GeometricContextLabel::Vertical: return OrientationContext::Vertical;
                case GeometricContextLabel::NotPlanar: return OrientationContext::NonPlanarObject;
                default:
                    return OrientationContext::Unknown;
                }
            }

            inline OrientationContext MakeOrientationContext(SizeI size, const std::vector<GeometricContextEstimator::Feature> & panoGCs){
                OrientationContext oc(size);
                ...
                for (int x = 0; x < size.width; x++){
                    for (int y = 0; y < size.height; y++){
                        ...
                        for (auto label : bestLabels){
                            oc.probs[ToOrientation(label)](y, x) += 1.0 / bestLabels.size();
                        }
        ...
        OrientationContext ComputeOrientationContext(const View<PanoramicCamera> & panoView, 
            ...
            for (int i = 0; i < perspectiveGCs.size(); i++){
                ...
                auto sampleBack = MakeCameraSampler(panoView.camera, perspectiveGCs[i].camera);
                for (auto & gcc : gc){
                    gcc.second = sampleBack(gcc.second, cv::BORDER_CONSTANT, 0);
                }
                gcs[i] = gc;
                //InstallGeometricContext(ops, gc);
            }
            return MakeOrientationContext(panoView.image.size(), gcs);
            //return MergeAndMakeOrientationContext(panoView.image.size(), ops);
        }
        ...
        OrientationContext ComputeOrientationContext(const View<PanoramicCamera> & panoView, SceneClass sceneClaz,
            ...
            auto cams = core::CreateHorizontalPerspectiveCameras(panoView.camera, num, width, height, focal);
            std::vector<View<PerspectiveCamera, GeometricContextEstimator::Feature>> perspectiveGCs(cams.size());

            for (int i = 0; i < cams.size(); i++){                
                Image sampled = MakeCameraSampler(cams[i], panoView.camera)(panoView.image, cv::BORDER_REPLICATE, 0);
                perspectiveGCs[i].image = gcEstimator(sampled, sceneClaz);
                perspectiveGCs[i].camera = cams[i];
            }

            return ComputeOrientationContext(panoView, perspectiveGCs);
        }

        OrientationContext ComputeOrientationContext(const View<PerspectiveCamera> & perspView, SceneClass sceneClaz,
            const GeometricContextEstimator & gcEstimator){
            std::map<OrientationContext::Orientation, std::vector<Imaged>> ops;
            auto gc = gcEstimator(perspView.image, sceneClaz);
            //InstallGeometricContext(ops, gc);
            //return  MergeAndMakeOrientationContext(perspView.image.size(), ops);
            return MakeOrientationContext(perspView.image.size(), { gc });
        }
