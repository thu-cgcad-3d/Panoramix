...
namespace panoramix {
    namespace core {
        ...
        namespace experimental {
            ...
            template <int ChildL, int N>
            struct ChildConfig {
                enum { ChildLevel = ChildL };
                enum { Num = N };
            };

            template <class ChildConfigT>
            struct ChildHandleStorage {
            };

            template <int ChildL, int N>
            struct ChildHandleStorage<ChildConfig<ChildL, N>> {
                using type = std::array<Handle<AtLevel<ChildL>>, N>;
            };

            template <int ChildL>
            struct ChildHandleStorage<ChildConfig<ChildL, Dynamic>> {
                using type = std::vector<Handle<AtLevel<ChildL>>>;
            };
