...
namespace pano {
    namespace experimental {
            ...
            for (int seg = 0; seg < mg.nsegs; seg++) {
                ...
                for (int k = 0; k < cs.size(); k++) {
                    ...
                    if (depth > 0) {
                        ...
                        compact.corners[k] = IntersectionOfLineAndPlane(Ray3(Origin(), dir), plane).position;
                    }
                }
        ...
        std::vector<Vec3> ComputeSegNormals(const PICGDeterminablePart & dp, const PIConstraintGraph & cg, const PIGraph & mg, bool smoothed) {
