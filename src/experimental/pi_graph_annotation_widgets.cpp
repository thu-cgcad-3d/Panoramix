    ...
    namespace experimental {
        ...
        PILayoutAnnotationWidget::PILayoutAnnotationWidget(QWidget * parent /*= nullptr*/) 
            ...
            // set mode
            {
                ...
                connect(_actPick = bas->addAction(tr("Pick Mode")), &QAction::triggered, [this]() {
                    _state = Pick;
                    clearStroke();
                    update();
                });
                _actPick->setShortcut(tr("q"));
                connect(_actDrawBorder = bas->addAction(tr("Draw Border")), &QAction::triggered, [this]() {
            ...
            addAction(defaultAction);
            connect(defaultAction = new QAction(tr("Make Face Free"), this), &QAction::triggered, [this]() {
                if (_faceClicked != -1) {
                    _anno->face2control[_faceClicked] = { -1, -1, true };
                    rebuildLayoutScene();
                }
            ...
            // save
            connect(defaultAction = new QAction(tr("Save Current Annotation"), this), &QAction::triggered, [this]() {
                if (_imagePath) {
        ...
        void PILayoutAnnotationWidget::setCurAnnotation(PILayoutAnnotation * anno, const QString * imagePath) {
            _state = Pick;
            _anno = anno;
        ...
        void PILayoutAnnotationWidget::rebuildLayoutScene() {
            ...
            for (int i = 0; i < _anno->face2control.size(); i++) {
                ...
                for (int corner : cs) {
                    facePolygonCenterLines.push_back(normalize(Line3(center, corners[corner])) * visualDepthFaceCenterLines);
                }
            }
            sb.begin(_facePolygons, [this](gui::InteractionID iid, const Decorated<Colored<Polygon3>, int> & polygon) {
            ...
            for (int i = 0; i < _anno->coplanarFacePairs.size(); i++) {
                ...
                _coplanarFacePairLines[i].component = 
                    normalize(Line3(facePolygonCenters[fp.first], facePolygonCenters[fp.second]) * visualDepthFaceCoplanarLine);
                _coplanarFacePairLines[i].decoration = i;
            }
            sb.installingOptions().discretizeOptions.color = gui::White;
            sb.begin(_coplanarFacePairLines, [this](gui::InteractionID iid, const Decorated<Line3, int> & line) {
