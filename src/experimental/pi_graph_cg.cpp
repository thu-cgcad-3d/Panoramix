...
namespace pano {
    namespace experimental {
        ...
        namespace {
            ...
            const T & AtOr(const std::map<K, T, Pr, Alloc> & m, const K & key, const T & defaultVal) {
                if (Contains(m, key)) {
                    return m.at(key);
                } else {
                    return defaultVal;
        ...
        PIConstraintGraph BuildPIConstraintGraph(const PIGraph & mg,
...

            {
                for (int line = 0; line < mg.nlines(); line++) {
                    ...
                    for (int seg1 : leftSegs) {
                        for (int seg2 : rightSegs) {
                            ...
                            segSeg2weightRatio[segPair] = std::min(AtOr(segSeg2weightRatio, segPair, 1.0), crossWR);
                        }
                    }
                    ...
                    for (int lr : mg.line2lineRelations[line]) {
                        ...
                        lineLine2weightRatio[linePair] = std::min(AtOr(lineLine2weightRatio, linePair, 1.0), wr);
                    }
                }
                ...
                for (int line = 0; line < mg.nlines(); line ++) {
                    ...
                    for (double a = cutLinePieceAngle; a <= spanAngle; a += cutLinePieceAngle) {
                        ...
                        linePiecesTree.emplace(BoundingBox(linePieceInst).expand(cutLinePieceAngle), line);
                        lastDir = dir;
                    }
                }

                const auto cutWR = [&linePiecesTree, &mg, &lsw](const Line3 & connection, const std::vector<int> & withdrawedLines) -> double {
                    double wr = 1.0;
                    ...
                    for (double a = 0.0; a <= spanAngle; a += cutLinePieceAngle) {
                        ...
                        linePiecesTree.search(BoundingBox(normalize(dir)).expand(3 * cutLinePieceAngle),
                            [&connection, &wr, &mg, &lsw, &withdrawedLines](const std::pair<Box3, int> & linePieceInst) {
                            if (Contains(withdrawedLines, linePieceInst.second)) { // the line should be withdrawed
                                return true;
                            }
                            auto & cutLine = mg.lines[linePieceInst.second].component;
                            Vec3 vertDir = normalize(cutLine.first.cross(cutLine.second));
                            if (connection.first.dot(vertDir) * connection.second.dot(vertDir) > 1e-5) { // not crossed
                                return true;
                            }
                            auto interp = normalize(IntersectionOfLineAndPlane(connection.ray(), Plane3(Origin(), vertDir)).position);
                            if (cutLine.first.cross(interp).dot(cutLine.second.cross(interp)) > 1e-5) { // not crossed
                                return true;
                            }
                ...
                for (int line = 0; line < mg.nlines(); line++) {
                    ...
                    for (int lp : mg.line2linePieces[line]) {
                        ...
                        if (bp != -1) {
                            ...
                            double wrToLeftSeg = cutWR(normalize(Line3(mg.lines[line].component.center(), mg.seg2center[seg1])), { line });
                            segLine2weightRatio[std::make_pair(seg1, line)] = 
                                std::min(AtOr(segLine2weightRatio, std::make_pair(seg1, line), 1.0), wrToLeftSeg);
                            double wrToRightSeg = cutWR(normalize(Line3(mg.lines[line].component.center(), mg.seg2center[seg2])), { line });
                            segLine2weightRatio[std::make_pair(seg2, line)] =
                                std::min(AtOr(segLine2weightRatio, std::make_pair(seg2, line), 1.0), wrToRightSeg);
                        } else {
                            int seg = mg.linePiece2seg[lp];
                            assert(seg != -1);
                            double wrToSeg = cutWR(normalize(Line3(mg.lines[line].component.center(), mg.seg2center[seg])), { line });
                            segLine2weightRatio[std::make_pair(seg, line)] =
                                std::min(AtOr(segLine2weightRatio, std::make_pair(seg, line), 1.0), wrToSeg);
                        }
                    }
                    // line-line
                    for (int lr : mg.line2lineRelations[line]) {
                        ...
                        double wr = cutWR(normalize(Line3(nearest.first.position, nearest.second.position)), { line, anotherLine });
                        auto linePair = MakeOrderedPair(line, anotherLine);
            ...
            for (int i = 0; i < mg.nbndPieces(); i++) {
                ...
                connect.ent2 = seg2ent[segPair.second];
                double len = mg.bndPiece2length[i];
                connect.weight = len * AtOr(segSeg2weightRatio, MakeOrderedPair(segPair.first, segPair.second), 0.5);
                if (len >= minAngleThresForAWideEdge) {
                    connect.anchors = { mg.bndPiece2dirs[i].front(), mg.bndPiece2dirs[i].back() };
                } else {
                    connect.anchors = { normalize(mg.bndPiece2dirs[i].front() + mg.bndPiece2dirs[i].back()) };
                ...
                if (mg.bndPiece2linePieces[i].empty()) {
                    ...
                    coplanar.ent1 = connect.ent1;
                    coplanar.ent2 = connect.ent2;
                    coplanar.weight = len * AtOr(segSeg2weightRatio, MakeOrderedPair(segPair.first, segPair.second), 0.5);
                    //coplanar.anchorOrientation = -1;
                    constraints.push_back(coplanar);
                    int coplanarCon = constraints.size() - 1;
                    ent2cons[coplanar.ent1].push_back(coplanarCon);
            ...
            for (int i = 0; i < mg.nlinePieces(); i++) {
                ...
                if (bndPiece == -1) {
                    ...
                    c.ent2 = line2ent[line];
                    double len = mg.linePiece2length[i];
                    c.weight = len * AtOr(segLine2weightRatio, std::make_pair(seg, line), 0.5);
                    if (len >= minAngleThresForAWideEdge) {
                        c.anchors = { mg.linePiece2samples[i].front(), mg.linePiece2samples[i].back() };
                    } else {
                        c.anchors = { normalize(mg.linePiece2samples[i].front() + mg.linePiece2samples[i].back()) };
            ...
            for (int i = 0; i < mg.nlineRelations(); i++) {
               /* if (mg.lineRelations[i] == LineRelation::Detached) {
                ...
                c.ent1 = line2ent[linePair.first];
                c.ent2 = line2ent[linePair.second];
                c.weight = mg.lineRelation2weight[i] * AtOr(lineLine2weightRatio, MakeOrderedPair(linePair.first, linePair.second), 0.5);
                c.anchors = { mg.lineRelation2anchor[i] };
                //c.anchorOrientation = -1;
                constraints.push_back(c);
                int con = constraints.size() - 1;
