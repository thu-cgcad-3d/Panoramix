    ...
    namespace experimental {
        ...
        void DetectOcclusions2(PIGraph & mg, double minAngleSizeOfLineInTJunction,
            ...
            for (int line = 0; line < mg.nlines(); line++) {             
                ...
                std::vector<LineLabel> allowedLabels = { { true, true }, { true, false }, { false, true } };
            ...
            std::vector<LineLabelCost> line2labelCost(mg.nlines(), { 0, 1e-8, 1e-8 });
            ...
            for (int line = 0; line < mg.nlines(); line++) {
                ...
                for (int lp : lps) {
                    double len = mg.linePiece2length[lp];
                    double weight = len * 5;
                    if (mg.linePiece2seg[lp] != -1) {
                        int seg = mg.linePiece2bndPiece[lp];
            ...
            if (true) {
                ...
                for (int line = 0; line < line2labelCost.size(); line++) {
                    ...
                    gui::Color color = Vec3(0.0, connectLeftCost, connectRightCost) / allCost;
                    drawLine(mg.lines[line].component, color);
                }
                gui::AsCanvas(pim).show();
            }
            ...
            for (int line = 0; line < mg.nlines(); line++) {
                ...
                int fc = fg.addFactorCategory([&line2labelCost, &mg, &line2allowedLabels, line](
                    ...
                    double costSum = labelCosts.connectLeftConnectRight + 
                        ...
                        labelCosts.disconnectLeftConnectRight + 1e-3;
            ...
            if (true) {
                ...
                for (int line = 0; line < line2labelCost.size(); line++) {
                    ...
                    gui::Color color = (label.connectLeft && !label.connectRight) ? 
                        gui::Green : ((label.connectRight && !label.connectLeft) ? gui::Blue : gui::Transparent);
                    drawLine(mg.lines[line].component, color);
                }
