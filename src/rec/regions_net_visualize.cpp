...
namespace panoramix {
    namespace rec {
        ...
        Visualizer2D operator << (Visualizer2D viz, const RegionsNet & net) {
            ...
            std::vector<cv::Vec<uint8_t, 3>> colors(net.regions().internalVertices().size());
            ...
            for (auto & h : net.regions().halfedges()){
                auto fromCenter = net.regions().data(h.topo.from()).center;
                auto toCenter = net.regions().data(h.topo.to()).center;
                Line2 line;
