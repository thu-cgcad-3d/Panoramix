...
namespace pano {
    namespace gui {
        ...
        namespace {
            inline void DrawRay2OnImage(Image & im, const Ray2 & ray, const PaintingOptions & po) {
        ...
        public:
            ...
            Canvas & color(const Color & c) { _paintingOptions.color = c; return *this; }
            Canvas & thickness(int t) { _paintingOptions.thickness = t; return *this; }
            Canvas & alpha(float a) { _paintingOptions.alpha = a; return *this; }
            Canvas & colorTable(const ColorTable & ct) { _paintingOptions.colorTable = ct; return *this; }
            ...
            template <class TT>
            inline Canvas & add(const HPoint<TT, 2> & p) {
                return add(p.value());
            }

            // lines
            template <class TT>
            inline Canvas & add(const Line<TT, 2> & line) {
            ...
            inline Canvas & add(const KeyPoint & p) {
                cv::drawKeypoints(image(), std::vector<KeyPoint>(1, p), image(), paintingOptions().color);
                return *this;
            }
            ...
            // classified thing
            template <class T>
            inline Canvas & add(const Classified<T> & thing) {
            ...
            template <class T>
            inline Canvas & add(std::initializer_list<T> a) {
                for (auto & e : a)
                    add(e);
                return *this;
            }
