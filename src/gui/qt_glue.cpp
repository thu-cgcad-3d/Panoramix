...
namespace panoramix {
    namespace gui {
        ...
        QImage MakeQImage(const cv::Mat & inMat) {
            ...
            return QImage(midMat.data, midMat.cols, midMat.rows, midMat.step, QImage::Format_RGB32);
        }
        ...
        cv::Mat MakeCVMat(const QImage & inImage, bool inCloneImageData) {
            ...
            default:
                qWarning() << "MakeCVMat() - QImage format not handled in switch:" << inImage.format();
                break;
            }
            QImage midImage = inImage.convertToFormat(QImage::Format_ARGB32);
            return cv::Mat(midImage.height(), midImage.width(), CV_8UC4,
                const_cast<uchar*>(midImage.bits()), midImage.bytesPerLine()).clone();
        }
