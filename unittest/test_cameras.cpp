...
TEST(Camera, CameraSampler) {
    ...
    std::vector<core::View<core::PartialPanoramicCamera, core::Image3ub>> ppanoViews;

    for (int i = 0; i < 4; i++){
        core::PerspectiveCamera cam(500, 600, core::Point2(250, 300), 150,
            core::Point3(0, 0, 0),
            core::Point3(camPositions[i][0], camPositions[i][1], camPositions[i][2]),
            core::Vec3(0, 0, -1));
        auto v = panoView.sampled(cam);
        perspViews.push_back(std::move(v));
        gui::AsCanvas(perspViews.back().image).show();

        core::PartialPanoramicCamera cam2(500, 600, 250, 
            core::Point3(0, 0, 0),
            core::Point3(camPositions[i][0], camPositions[i][1], camPositions[i][2]),
            panoView.camera.up());
